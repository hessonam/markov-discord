...
class Markov {
    ...
    generate(options = {}) {
        ...
        for (tries = 1; tries <= maxTries; tries++) {
            ...
            let arr = [];
            if (options.prompt && tries <= 10) {
              let startWordsPrompt = this.startWords.filter(function(startWord) {
                return startWord.words.toLowerCase().startsWith(options.prompt.toLowerCase());
              });
              if (startWordsPrompt.length === 0) {
                continue;
              }
              arr = [sampleWithPRNG(startWordsPrompt, prng)];
            } else {
              arr = [sampleWithPRNG(this.startWords, prng)];
            }

            // const arr = [{words: 'btc is'}];
            let score = 0;
